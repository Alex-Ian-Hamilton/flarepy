...
def _filter_ridge_lines(cwt, ridge_lines, window_size=None, min_length=None,
    ...
    Returns
    -------
    list(filter(filt_func, ridge_lines))
...
def find_peaks_cwt(vector,
    ...
    # Pre-calculated details - For speeding up re-calculations

    cwt_image : ndarray, optional
        Pre-calculated CWT image, this is to save time on rerunning code.
        This image should either include all widths upto the maximum given in
        the input widths sequence, or teh exact widths used in that sequence in
        order.

    ridge_lines : list, optional


    Returns
    -------
    max_locs, cwt_dat, ridge_lines, filtered
    ...
    # Check if pre-calculated CWT image was given
    if cwt_image is None:
        # No CWT data image given, calculate
        print('vector: '+str(vector))
        print('wavelet: '+str(wavelet))
        print('widths: '+str(widths))
        cwt_dat = cwt(vector, wavelet, widths)
    elif cwt_image.shape[0] > len(widths):
        # We are given a full CWT image, larger then the number of widths, so
        # filter to match the widths
        cwt_dat = cwt_image[widths]
    else:
        # We are given matching CWT image and widths
        cwt_dat = cwt_image

    # Check if pre-calculated ridge-lines were given
    if ridge_lines is None:
        # Not given, so calculate
...
def get_flare_peaks_cwt(ser_data,
                         widths=np.arange(1,100),# CWT
                         raw_data=None,
                         get_duration=True,
                         get_energies=True,
                         ser_minima=None,
    ...
    arr_peak_time_indices, cwt_dat, ridge_lines, filtered = find_peaks_cwt(ser_data.values,
                                                                                    widths,
                                                                                    wavelet=wavelet,
    ...
    pd_peaks_cwt = pd.DataFrame(data={'event_peaktime': ser_data.index[arr_peak_time_indices],
                                      'i_index': arr_peak_time_indices},
                                index=ser_data.index[arr_peak_time_indices])
    pd_peaks_cwt['fl_peakflux'] = ser_data[arr_peak_time_indices]
    pd_peaks_cwt['fl_goescls'] = utils.arr_to_cla(pd_peaks_cwt['fl_peakflux'].values, int_dp=1)
    """
    print('arr_peak_time_indices: '+str(arr_peak_time_indices))
    ser_cwt_peaks = ser_raw_data[arr_peak_time_indices]
    print('ser_cwt_peaks: ' + str(ser_cwt_peaks))
