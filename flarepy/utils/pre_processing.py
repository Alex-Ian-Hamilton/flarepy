...
def pre_process(data, resample_bins=60*u.s, resample_method='median', average_method='mean', int_cart=5):
    ...
    Returns
    -------
    ser_raw_int_res_box : ~`pandas.Series`
        The final data series that has been resampled/rebinned and averaged.

    ser_raw_int_res : ~`pandas.Series`
        The data series that has been resampled/rebinned but not averaged.

    ser_plt_fil : ~`pandas.Series`
        The data series that has been resampled/rebinned and averaged with NaN
        values for data gaps in the original dataset.
    """
    # Sanitise arguments
