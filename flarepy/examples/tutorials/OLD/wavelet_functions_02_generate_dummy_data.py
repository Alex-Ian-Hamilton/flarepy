...
for i, in_width in enumerate([30, 50, 70, 90, 110]):
    ...
    # Save the figure
    #figure_a.show()
    str_save = 'single-wavelet__single-width_CWT\\single-wavelet__single-width_CWT___ricker('+str(parameters[0][0])+', '+str(parameters[0][1])+')___widths_['+str(single_widths[0])+']_['+str(single_widths[1])+']_['+str(single_widths[2])+']_['+str(single_widths[3])+']_['+str(single_widths[4])+'].png'
    figure_1.savefig(str_save, dpi=900)
...
for i, in_width in enumerate([30, 50, 70, 90, 110]):
    ...
    str_file_prefix = 'single-wavelet__3-value-widths_CWT___0.3333-increments'
    # Quarters
    lis_widths = [
            [ in_width],
    ...
    str_file_prefix = 'single-wavelet__3-value-widths_CWT___0.2500-increments'
    # Tenths
    lis_widths = [
            [ in_width],
    ...
    str_file_prefix = 'single-wavelet__3-value-widths_CWT___0.1000-increments'

    for j, widths in enumerate(lis_widths):
        arr_cwt_det = signal.find_peaks_cwt(arr_data, widths)
        axes_2.plot(arr_cwt_det, arr_data[arr_cwt_det], lis_markers[j], label='#'+str(len(arr_cwt_det))+' peaks for width '+str(widths)+'', markersize=5, lw=3)#), color=conf[3], markersize=conf[2], lw=2, zorder=conf[6])
    ...
    # Save the figure
    #figure_a.show()
    str_save = 'single-wavelet__3-value-widths_CWT\\'+str_file_prefix+'___ricker('+str(parameters[0][0])+', '+str(parameters[0][1])+')_CWT_multi-widths_'+str(lis_widths[0])+'_'+str(lis_widths[1])+'_'+str(lis_widths[2])+'.png'
    figure_2.savefig(str_save, dpi=900)
...
for i, in_width in enumerate([30, 50, 70, 90, 110]):
    ...
    #figure_a.show()
    str_save = 'single-wavelet__many-widths_CWT___ricker('+str(parameters[0][0])+', '+str(parameters[0][1])+')_CWT_multi-widths_'+str(lis_widths[0])+'_'+str(lis_widths[1])+'_'+str(lis_widths[2])+'.png'
