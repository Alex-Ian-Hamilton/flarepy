...
def modified_cwt(data, wavelet, widths):
    """
...
def convolve(in1, in2, mode='full', method='auto'):
    ...
    if volume.ndim == kernel.ndim == 0:
        ...
        print('\nvolume.ndim == kernel.ndim == 0, so the convolution is volume * kernel.\n')
    ...
    if method == 'fft':
        # Check if this is activated
        print('\nConvolution method == \'fft\', using fftconvolve() method.\n')
    ...
    if _np_conv_ok(volume, kernel, mode):
        # Check if this is activated
        print('\n_np_conv_ok returned true, using np.convolve() method.\n')
        return np.convolve(volume, kernel, mode)

    # Otherwise
    # Check if this is activated
    print('\nConvolution uses np.correlate(volume, _reverse_and_conj(kernel), mode, \'direct\') method.\n')
...
def modified_find_peaks_cwt(vector, widths, wavelet=None, max_distances=None, gap_thresh=None,
    ...
    #
    cwt_dat = modified_cwt(vector, wavelet, widths)
...
arr_data = np.sin(np.arange(int_datapoints) * np.pi / 30. )
str_function = 'sinusoid'

#arr_data = signal.ricker(int_datapoints, 50)
#str_function = 'ricker, a=50'

#arr_data = signal.morlet(int_datapoints, 10)
#str_function = 'morlet, a=10'

#arr_data = signal.exponential(int_datapoints)
#str_function = 'exponential'

#arr_data = signal.gaussian(int_datapoints, std=50)
#str_function = 'gaussian, std=50'

# Get the CWT peaks
arr_peaks, cwt_dat, ridge_lines, filtered = modified_find_peaks_cwt(arr_data, arr_cwt_widths)
